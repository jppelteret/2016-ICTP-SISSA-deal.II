...
namespace Step37
{
  ...
  void LaplaceProblem<dim>::compare_performance_matrix_vector() const
  {
    ...
    const unsigned int n_products = 100;
#ifdef LIKWID_PERFMON
    LIKWID_MARKER_START("matrix_free_op");
#endif
    Timer time;
...
#ifdef LIKWID_PERFMON
    LIKWID_MARKER_STOP("matrix_free_op");
#endif

#ifdef LIKWID_PERFMON
    LIKWID_MARKER_START("sparse_matrix_op");
#endif
    time.restart();
    for (unsigned int i=0; i<n_products; ++i)
...
#ifdef LIKWID_PERFMON
    LIKWID_MARKER_STOP("sparse_matrix_op");
#endif
